...
import { Motorcycle, Part, PartCondition, MotorcycleCategory, PartCategory } from '../types';
...
const motorcycleCategories: Exclude<MotorcycleCategory, 'All'>[] = ['Sport', 'Cruiser', 'Off-Road', 'Touring'];
const partCategories: Exclude<PartCategory, 'All'>[] = ['Exhausts', 'Brakes', 'Tires', 'Suspension', 'Electronics'];
...
const SellForm: React.FC<SellFormProps> = ({ onBack, onPublish }) => {
  ...
  // Moto state
  const [motoData, setMotoData] = useState({ make: '', model: '', year: '', mileage: '', engineSize: '', category: 'Sport' as Exclude<MotorcycleCategory, 'All'> });
  
  // Part state
  const [partData, setPartData] = useState({ name: '', condition: 'new' as PartCondition, compatibility: '', category: 'Exhausts' as Exclude<PartCategory, 'All'> });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (listingType === 'motorcycle') {
        ...
        if (!make || !model || !year || !price || !mileage || !engineSize || !description || !location || !category) {
        ...
        const engineSizeNum = parseInt(engineSize, 10);
        ...
        if (!name || !price || !description || !location || !compatibility || !category) {
        ...
        if (compatibility.length < 3) {
            alert('Por favor, introduce información de compatibilidad válida.');
            return;
        }
        
        onPublish({
            name, location, price: priceNum, description, videoUrl: videoUrl || undefined,
            condition,
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {listingType === 'motorcycle' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <select name="category" onChange={(e) => setMotoData(p => ({...p, category: e.target.value as Exclude<MotorcycleCategory, 'All'>}))} value={motoData.category} className="form-input category-select" required>
                {motorcycleCategories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
            </div>
        ) : (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <select name="category" onChange={(e) => setPartData(p => ({...p, category: e.target.value as Exclude<PartCategory, 'All'>}))} value={partData.category} className="form-input md:col-span-2" required>
      ...
      <style>{`.form-input { width: 100%; background-color: transparent; border: 1px solid #2a3c46; border-radius: 0.75rem; padding: 0.75rem 1rem; color: inherit; transition: all 0.2s; } .form-input:focus { outline: none; border-color: #1193d4; box-shadow: 0 0 0 2px rgba(17, 147, 212, 0.5); } select.form-input { padding-right: 2.5rem; background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e"); background-position: right 0.5rem center; background-repeat: no-repeat; background-size: 1.5em 1.5em; appearance: none; } .category-select { position: relative; z-index: 1000; } select.form-input:focus { z-index: 1001; } select.form-input option { color: #1e1e1e; background-color: #ffffff; } .dark select.form-input option { color: #f8f9fa; background-color: #2c2c2c; }`}</style>
    </div>
  );
};
